#!/usr/bin/env python3
"""
VectorAI MCP Client - Enhanced AI Agent Communication Interface
...
Architecture: MCP Client for AI agent communication with VectorAI server
...
Colors = VectorAIColors

class ColoredFormatter(logging.Formatter):
    """Enhanced formatter with colors and emojis for MCP client - matches server styling"""

    COLORS = {
        'DEBUG': VectorAIColors.DEBUG,
        'INFO': VectorAIColors.SUCCESS,
        'WARNING': VectorAIColors.WARNING,
        'ERROR': VectorAIColors.ERROR,
        'CRITICAL': VectorAIColors.CRITICAL
    }
    ...
    def format(self, record):
        ...
        color = self.COLORS.get(record.levelname, VectorAIColors.BRIGHT_WHITE)

        # Add color and emoji to the message
        record.msg = f"{color}{emoji} {record.msg}{VectorAIColors.RESET}"
        return super().format(record)

# Setup logging
logging.basicConfig(
    level=logging.INFO,
    format="[🔥 VectorAI MCP] %(asctime)s [%(levelname)s] %(message)s",
    handlers=[
...
for handler in logging.getLogger().handlers:
    handler.setFormatter(ColoredFormatter(
        "[🔥 VectorAI MCP] %(asctime)s [%(levelname)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    ))
...
DEFAULT_VECTORAI_SERVER = "http://127.0.0.1:8888"  # Default VectorAI server URL
DEFAULT_REQUEST_TIMEOUT = 300  # 5 minutes default timeout for API requests
MAX_RETRIES = 3  # Maximum number of retries for connection attempts

class VectorAIClient:
    """Enhanced client for communicating with the VectorAI API Server"""
    ...
    def __init__(self, server_url: str, timeout: int = DEFAULT_REQUEST_TIMEOUT):
        ...
        Args:
            server_url: URL of the VectorAI API Server
        ...
        for i in range(MAX_RETRIES):
            try:
                logger.info(f"🔗 Attempting to connect to VectorAI API at {server_url} (attempt {i+1}/{MAX_RETRIES})")
                ...
                try:
                    ...
                    connected = True
                    logger.info(f"🎯 Successfully connected to VectorAI API Server at {server_url}")
                    ...
                    logger.warning(f"🔌 Connection refused to {server_url}. Make sure the VectorAI server is running.")
        ...
        if not connected:
            error_msg = f"Failed to establish connection to VectorAI API Server at {server_url} after {MAX_RETRIES} attempts"
    ...
    def execute_command(self, command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute a generic command on the VectorAI server
    ...
    def check_health(self) -> Dict[str, Any]:
        """
        Check the health of the VectorAI API Server
...
def setup_mcp_server(VectorAI_client: VectorAIClient) -> FastMCP:
    ...
    def nmap_scan(target: str, scan_type: str = "-sV", ports: str = "", additional_args: str = "") -> Dict[str, Any]:
        ...
        logger.info(f"{VectorAIColors.FIRE_RED}🔍 Initiating Nmap scan: {target}{VectorAIColors.RESET}")
        ...
        if result.get("success"):
            ...
            if result.get("recovery_info", {}).get("recovery_applied"):
                ...
                logger.info(f"{VectorAIColors.HIGHLIGHT_YELLOW} Recovery applied: {attempts} attempts made {VectorAIColors.RESET}")
        else:
            logger.error(f"{VectorAIColors.ERROR}❌ Nmap scan failed for {target}{VectorAIColors.RESET}")

            # Check for human escalation
            if result.get("human_escalation"):
                logger.error(f"{VectorAIColors.CRITICAL} HUMAN ESCALATION REQUIRED {VectorAIColors.RESET}")
    ...
    def gobuster_scan(url: str, mode: str = "dir", wordlist: str = "/usr/share/wordlists/dirb/common.txt", additional_args: str = "") -> Dict[str, Any]:
        ...
        logger.info(f"{VectorAIColors.CRIMSON}📁 Starting Gobuster {mode} scan: {url}{VectorAIColors.RESET}")
        ...
        if result.get("success"):
            ...
            logger.error(f"{VectorAIColors.ERROR}❌ Gobuster scan failed for {url}{VectorAIColors.RESET}")

            # Check for alternative tool suggestion
            if result.get("alternative_tool_suggested"):
                alt_tool = result["alternative_tool_suggested"]
                logger.info(f"{VectorAIColors.HIGHLIGHT_BLUE} Alternative tool suggested: {alt_tool} {VectorAIColors.RESET}")
    ...
    def nuclei_scan(target: str, severity: str = "", tags: str = "", template: str = "", additional_args: str = "") -> Dict[str, Any]:
        ...
        logger.info(f"{VectorAIColors.BLOOD_RED}🔬 Starting Nuclei vulnerability scan: {target}{VectorAIColors.RESET}")
        ...
        if result.get("success"):
            ...
            if result.get("stdout") and "CRITICAL" in result["stdout"]:
                logger.warning(f"{VectorAIColors.CRITICAL} CRITICAL vulnerabilities detected! {VectorAIColors.RESET}")
            elif result.get("stdout") and "HIGH" in result["stdout"]:
                logger.warning(f"{VectorAIColors.FIRE_RED} HIGH severity vulnerabilities found! {VectorAIColors.RESET}")
            ...
            logger.error(f"{VectorAIColors.ERROR}❌ Nuclei scan failed for {target}{VectorAIColors.RESET}")
    ...
    @mcp.tool()
    def pacu_exploitation(session_name: str = "VectorAI_session", modules: str = "",
    ...
    def create_file(filename: str, content: str, binary: bool = False) -> Dict[str, Any]:
        """
        Create a file with specified content on the VectorAI server.
    ...
    def modify_file(filename: str, content: str, append: bool = False) -> Dict[str, Any]:
        """
        Modify an existing file on the VectorAI server.
    ...
    def delete_file(filename: str) -> Dict[str, Any]:
        """
        Delete a file or directory on the VectorAI server.
    ...
    def list_files(directory: str = ".") -> Dict[str, Any]:
        """
        List files in a directory on the VectorAI server.
    ...
    def install_python_package(package: str, env_name: str = "default") -> Dict[str, Any]:
        """
        Install a Python package in a virtual environment on the VectorAI server.
    ...
    def execute_python_script(script: str, env_name: str = "default", filename: str = "") -> Dict[str, Any]:
        """
        Execute a Python script in a virtual environment on the VectorAI server.
    ...
    @mcp.tool()
    def ghidra_analysis(binary: str, project_name: str = "VectorAI_analysis",
    ...
    def server_health() -> Dict[str, Any]:
        """
        Check the health status of the VectorAI server.
    ...
    def get_cache_stats() -> Dict[str, Any]:
        """
        Get cache statistics from the VectorAI server.
    ...
    def clear_cache() -> Dict[str, Any]:
        """
        Clear the cache on the VectorAI server.
    ...
    def get_telemetry() -> Dict[str, Any]:
        """
        Get system telemetry from the VectorAI server.
    ...
    def list_active_processes() -> Dict[str, Any]:
        """
        List all active processes on the VectorAI server.
    ...
    def execute_command(command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute an arbitrary command on the VectorAI server with enhanced logging.
...
""",
    ...
    def intelligent_smart_scan(target: str, objective: str = "comprehensive", max_tools: int = 5) -> Dict[str, Any]:
        ...
        """
        logger.info(f"{VectorAIColors.FIRE_RED}🚀 Starting intelligent smart scan for {target}{VectorAIColors.RESET}")
        ...
        if result.get("success"):
            ...
            logger.info(f"{VectorAIColors.SUCCESS}✅ Intelligent scan completed for {target}{VectorAIColors.RESET}")
            logger.info(f"{VectorAIColors.CYBER_ORANGE}📊 Execution Summary:{VectorAIColors.RESET}")
            ...
            if successful_tools:
                logger.info(f"{VectorAIColors.HIGHLIGHT_GREEN} Successful tools: {', '.join(successful_tools)} {VectorAIColors.RESET}")

            # Log failed tools
            failed_tools = [t['tool'] for t in tools_executed if not t.get('success')]
            if failed_tools:
                logger.warning(f"{VectorAIColors.HIGHLIGHT_RED} Failed tools: {', '.join(failed_tools)} {VectorAIColors.RESET}")

            # Log vulnerabilities found
            if scan_results.get('total_vulnerabilities', 0) > 0:
                logger.warning(f"{VectorAIColors.VULN_HIGH}🚨 {scan_results['total_vulnerabilities']} vulnerabilities detected!{VectorAIColors.RESET}")
        else:
            logger.error(f"{VectorAIColors.ERROR}❌ Intelligent scan failed for {target}: {result.get('error', 'Unknown error')}{VectorAIColors.RESET}")
    ...
    def http_framework_test(url: str, method: str = "GET", data: dict = {},
        ...
        if result.get("success"):
            ...
            if result.get("result", {}).get("vulnerabilities"):
                ...
                logger.info(f"{VectorAIColors.HIGHLIGHT_RED} Found {vuln_count} potential vulnerabilities {VectorAIColors.RESET}")
        else:
            logger.error(f"{VectorAIColors.ERROR}❌ HTTP Framework {action} failed for {url}{VectorAIColors.RESET}")
    ...
    def browser_agent_inspect(url: str, headless: bool = True, wait_time: int = 5,
        ...
        if result.get("success"):
            ...
            if action == "navigate" and result.get("result", {}).get("security_analysis"):
                ...
                if issues_count > 0:
                    logger.warning(f"{VectorAIColors.HIGHLIGHT_YELLOW} Security Issues: {issues_count} | Score: {security_score}/100 {VectorAIColors.RESET}")
                else:
                    logger.info(f"{VectorAIColors.HIGHLIGHT_GREEN} No security issues found | Score: {security_score}/100 {VectorAIColors.RESET}")
        else:
            logger.error(f"{VectorAIColors.ERROR}❌ Browser Agent {action} failed for {url}{VectorAIColors.RESET}")
    ...
    def burpsuite_alternative_scan(target: str, scan_type: str = "comprehensive",
        ...
        if result.get("success"):
            ...
            if result.get("result", {}).get("summary"):
                ...
                logger.info(f"{VectorAIColors.HIGHLIGHT_BLUE} SCAN SUMMARY {VectorAIColors.RESET}")
                ...
                for severity, count in vuln_breakdown.items():
                    if count > 0:
                        color = {
                                    'critical': VectorAIColors.CRITICAL,
        'high': VectorAIColors.FIRE_RED,
        'medium': VectorAIColors.CYBER_ORANGE,
        'low': VectorAIColors.YELLOW,
        'info': VectorAIColors.INFO
    }.get(severity.lower(), VectorAIColors.WHITE)

                        logger.info(f"  {color}{severity.upper()}: {count}{VectorAIColors.RESET}")
        else:
            logger.error(f"{VectorAIColors.ERROR}❌ Burp Suite Alternative scan failed for {target}{VectorAIColors.RESET}")
    ...
    def error_handling_statistics() -> Dict[str, Any]:
        ...
        if result.get("success"):
            ...
            logger.info(f"{VectorAIColors.SUCCESS}✅ Error statistics retrieved{VectorAIColors.RESET}")
            ...
            if error_counts:
                logger.info(f"{VectorAIColors.HIGHLIGHT_BLUE} ERROR BREAKDOWN {VectorAIColors.RESET}")
                for error_type, count in error_counts.items():
                                          logger.info(f"  {VectorAIColors.FIRE_RED}{error_type}: {count}{VectorAIColors.RESET}")
        else:
            logger.error(f"{VectorAIColors.ERROR}❌ Failed to retrieve error statistics{VectorAIColors.RESET}")
    ...
    def test_error_recovery(tool_name: str, error_type: str = "timeout",
        ...
        if result.get("success"):
            ...
            logger.info(f"{VectorAIColors.SUCCESS}✅ Error recovery test completed{VectorAIColors.RESET}")
            ...
            logger.error(f"{VectorAIColors.ERROR}❌ Error recovery test failed{VectorAIColors.RESET}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run the VectorAI MCP Client")
    parser.add_argument("--server", type=str, default=DEFAULT_VECTORAI_SERVER,
                      help=f"VectorAI API server URL (default: {DEFAULT_VECTORAI_SERVER})")
...
def main():
    ...
    logger.info(f"🚀 Starting VectorAI MCP Client v6.0")
    ...
    try:
        ...
        if "error" in health:
            ...
            logger.info(f"🎯 Successfully connected to VectorAI API server at {args.server}")
            logger.info(f"🏥 Server health status: {health['status']}")
            logger.info(f"📊 Version: {health.get('version', 'unknown')}")
            if not health.get("all_essential_tools_available", False):
                logger.warning("⚠️  Not all essential tools are available on the VectorAI server")
